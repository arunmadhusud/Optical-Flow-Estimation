...
The main.py file provided by authors is updated for saving the tarining and validation loss values in a txt file.
Only the respective comments are added in the main.py file.
the txt files are loaded later for plotting the loss curves.

Replace the 'main.py' provided by authors with this file to obtain the loss values in txt files
...
if __name__ == '__main__':
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            a=tools.format_dictionary_of_losses(loss_labels, statistics[-1])
            losses = a.split(", ")
            l1 = float(losses[0].split(": ")[1])
            epe = float(losses[1].split(": ")[1])
            
            # update the loss values in the text file
            if is_validate:
              with open("validation_loss_values.txt", "a") as f:
                f.write(str(epoch)+ ", ")
                f.write(str(batch_idx) + ", ")
                f.write(str(l1) + ", ")
                f.write(str(epe) + '\n')                
            else:
              with open("training_loss_values.txt", "a") as f:
                f.write(str(epoch)+ ", ")
                f.write(str(batch_idx) + ", ")
                f.write(str(l1) + ", ")
                f.write(str(epe) + '\n')                
    ...
    global_iteration = 0
    # open file to write training and validation loss values
    ...
    with open("validation_loss_values.txt", "w") as f:
              f.write("Epoch" + ", ")
              f.write("batch_idx" + ", ")
              f.write("L1 " + ", ")
              f.write("EPE" + '\n')
